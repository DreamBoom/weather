...
class F55 : LazyFragment(){
    override fun getLayoutResource(): Int = R.layout.f55
    ...
    fun setData() {
        ...
        runOnUiThread {
            time!!.text = "日期 ${reversed[0].time} 至 ${reversed[reversed.size - 1].time} 单位:Lux"
        }
        ...
        for (i in reversed.indices) {
            ...
            try {
                values.add(Entry(i.toFloat(), reversed[i].gz.toFloat()))
            } catch (e: Exception) {
                values.add(Entry(0L.toFloat(), reversed[i].gz.toFloat()))
            }
        }
    ...
    fun isShow():Boolean{
        return  this@F55.isResumed && this@F55.userVisibleHint
    }
}