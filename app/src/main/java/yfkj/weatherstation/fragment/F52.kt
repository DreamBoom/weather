...
class F52 : LazyFragment(){
    override fun getLayoutResource(): Int = R.layout.f52
    ...
    fun setData() {
        ...
        for (i in reversed.indices) {
            ...
            try {
                values.add(Entry(i.toFloat(), reversed[i].sd.toFloat()))
            } catch (e: Exception) {
                values.add(Entry(0L.toFloat(), reversed[i].sd.toFloat()))
            }
        }
    ...
    fun isShow():Boolean{
        return  this@F52.isResumed && this@F52.userVisibleHint
    }
}