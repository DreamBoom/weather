...
class F53 : LazyFragment(){
    override fun getLayoutResource(): Int = R.layout.f53
    ...
    fun setData() {
        ...
        runOnUiThread {
            time!!.text = "日期 ${reversed[0].time} 至 ${reversed[reversed.size - 1].time} 单位:℃"
        }
        ...
        for (i in reversed.indices) {
            ...
            try {
                values.add(Entry(i.toFloat(), reversed[i].tw.toFloat()))
            } catch (e: Exception) {
                values.add(Entry(0L.toFloat(), reversed[i].tw.toFloat()))
            }
        }
    ...
    fun isShow():Boolean{
        return  this@F53.isResumed && this@F53.userVisibleHint
    }
}