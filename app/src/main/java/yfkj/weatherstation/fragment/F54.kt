...
class F54 : LazyFragment(){
    override fun getLayoutResource(): Int = R.layout.f54
    ...
    fun setData() {
        ...
        for (i in reversed.indices) {
            ...
            try {
                values.add(Entry(i.toFloat(), reversed[i].ts.toFloat()))
            } catch (e: Exception) {
                values.add(Entry(0L.toFloat(), reversed[i].ts.toFloat()))
            }
        }
    ...
    fun isShow():Boolean{
        return  this@F54.isResumed && this@F54.userVisibleHint
    }
}