...
class F56 : LazyFragment(){
    override fun getLayoutResource(): Int = R.layout.f56
    ...
    fun setData() {
        ...
        runOnUiThread {
            time!!.text = "日期 ${reversed[0].time} 至 ${reversed[reversed.size - 1].time} 单位:Hpa"
        }
        ...
        for (i in reversed.indices) {
            ...
            try {
                values.add(Entry(i.toFloat(), reversed[i].qy.toFloat()))
            } catch (e: Exception) {
                values.add(Entry(0L.toFloat(), reversed[i].qy.toFloat()))
            }
        }
    ...
    fun isShow():Boolean{
        return  this@F56.isResumed && this@F56.userVisibleHint
    }
}