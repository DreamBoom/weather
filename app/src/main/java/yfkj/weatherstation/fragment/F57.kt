...
class F57 : LazyFragment(){
    override fun getLayoutResource(): Int = R.layout.f57
    ...
    fun setData() {
        ...
        runOnUiThread {
            time!!.text = "日期 ${reversed[0].time} 至 ${reversed[reversed.size - 1].time} 单位:mm/min"
        }
        ...
        for (i in reversed.indices) {
            ...
            try {
                values.add(Entry(i.toFloat(), reversed[i].yl.toFloat()))
            } catch (e: Exception) {
                values.add(Entry(0L.toFloat(), reversed[i].yl.toFloat()))
            }
        }
    ...
    fun isShow():Boolean{
        return  this@F57.isResumed && this@F57.userVisibleHint
    }
}